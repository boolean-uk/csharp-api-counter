...
namespace api_counter.wwwapi.Controllers
{
    ...
    public class CounterController : ControllerBase
    {
        ...
        {
            var result = counters.Where(x => x.Value >= number);
        ...
        {
            var result = counters.FirstOrDefault(x => x.Id == id);
            if (result != null)
            {
                result.Value++;
            }
            return result != null ? TypedResults.Ok(result) : TypedResults.NotFound(id);
        }
