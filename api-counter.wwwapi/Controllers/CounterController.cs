...
namespace api_counter.wwwapi.Controllers
{
    ...
    public class CounterController : ControllerBase
    {
        ...
        public async Task<IResult> GetGreaterThan([FromRoute] int number)
        {
        ...
        public async Task<IResult> GetLesserThan([FromRoute] int number)
        {
            var counter = counters.Where(x => x.Value < number);
            return TypedResults.Ok(counter);
        }
        ...
        [HttpGet]
        [Route("increment/{id}")]
        public async Task<IResult> IncrementACounter([FromRoute] int id)
        {
            counters.FirstOrDefault(x => x.Id == id).Value += 1;
            var counter = counters.FirstOrDefault(x => x.Id == id);
            return counter != null ? TypedResults.Ok(counter) : TypedResults.NotFound();
        }
        ...
        [HttpGet]
        [Route("decrement/{id}")]
        public async Task<IResult> DecrementACounter([FromRoute] int id)
        {
            counters.FirstOrDefault(x => x.Id == id).Value -= 1;
            var counter = counters.FirstOrDefault(x => x.Id == id);
            return counter != null ? TypedResults.Ok(counter) : TypedResults.NotFound();
