...
namespace api_counter.wwwapi.Controllers
{
    ...
    public class CounterController : ControllerBase
    {
        ...
        public async Task<IResult> GetAllCounters()
        {
            ...
            return CounterController.counters.Count != 0 ? TypedResults.Ok(CounterController.counters) : TypedResults.NotFound();
        }
        ...
        public async Task<IResult> GetACounter(int id)
        {
            ...
            var counter = CounterController.counters.FirstOrDefault(c => c.Id == id);
        ...
        {
            List<Counter> res = CounterController.counters.Where(c => c.Value > number).ToList();
            return res.Count != 0 ? TypedResults.Ok(res) : TypedResults.NotFound();
        }
        ...
        [HttpGet]
        [Route("increment/{id}")]
        public async Task<IResult> GetIncrementedCounter(int id)
        {
            Counter? counter = CounterController.counters.Where(c => c.Id == id).FirstOrDefault();
            if (counter != null)
            {
                counter.Value++;
        ...
        [HttpGet]
        [Route("decrement/{id}")]
        public async Task<IResult> GetDecrementedCounter(int id) 
        {
            Counter? counter = CounterController.counters.Where(c => c.Id == id).FirstOrDefault();
            if (counter != null)
            {
