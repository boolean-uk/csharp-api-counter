...
namespace api_counter.wwwapi.Controllers
{
    ...
    public class CounterController : ControllerBase
    {
        ...
        public async Task<IResult> Get(int number)
        {
            ...
            {
                if (counters[i].Value > number)
                    biggerCounters.Add(counters[i]);
            }
        ...
        public async Task<IResult> GetLess(int number)
        {
            ...
            {
                if (counters[i].Value < number)
                    smallerCounters.Add(counters[i]);
            }
        ...
        {
            if (id < 0 || id > counters.Count)
                return TypedResults.NotFound();

            counters[id].Value++;
            return TypedResults.Ok(counters[id]);
        }
        ...
        [HttpGet]
        [Route("Decrease/{id}")]
        public async Task<IResult> DrecreaseValue(int id)
        {
            if (id < 0 || id > counters.Count)
                return TypedResults.NotFound();
