...
namespace api_counter.wwwapi.Controllers
{
    ...
    public class CounterController : ControllerBase
    {
        ...
        public async Task<IResult> GreaterThan(int number)
        {
            return TypedResults.Ok(counters.Where(x => x.Value > number).ToList());
        }
        ...
        {
            Counter? c = counters.Where(x => x.Id == id).FirstOrDefault();
            if (c != null)
            {
                c.Value++;
