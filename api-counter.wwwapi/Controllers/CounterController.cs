...
namespace api_counter.wwwapi.Controllers
{
    ...
    public class CounterController : ControllerBase
    {
        ...
        public async Task<IResult> GetAllCountersGreaterThan(int number)
        {
            return TypedResults.Ok(counters.FindAll(x => x.Value > number));
        }

        ////TODO:4. write another controlller method that returns counters that have a value less than the {number} passed in.
        [HttpGet]
        [Route("lessthan/{number}")]
        public async Task<IResult> GetAllCountersLessThan(int number)
        {
            return TypedResults.Ok(counters.FindAll(x => x.Value < number));
        }
