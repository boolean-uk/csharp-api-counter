...
namespace api_counter.wwwapi.Controllers
{
    ...
    public class CounterController : ControllerBase
    {
        ...
        public async Task<IResult> GetACounter(int id)
        {
            ...
            var counter = counters[counters.FindIndex(c => c.Id == id)];
        ...
        {
            List<Counter> resultingCounters = counters.Where(counter => counter.Value > number).ToList();
            return TypedResults.Ok(resultingCounters);
        }
        ...
        {
            Counter counter= counters[counters.FindIndex(c => c.Id == id)];
            if (counter != null)
            {
                counter.Value++;
                return TypedResults.Created($"https://localhost:7293/counters/{id}", counter);
            }
