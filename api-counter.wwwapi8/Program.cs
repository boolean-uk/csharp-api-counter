...
counters.MapGet("/{id}", (int id) =>
{
    ...
    if(counter == null)
    {
        return Results.NotFound($"Counter with {id} not found");
    }
...
{
    var idToIncrement = CounterHelper.Counters.FirstOrDefault(idToIncrement => idToIncrement.Id == id);
...
{
    var idToDecrement = CounterHelper.Counters.FirstOrDefault(idToDecrement => idToDecrement.Id == id);
    if (idToDecrement == null)
    {
        return Results.NotFound($"Id: {id} not found!");
    }
    idToDecrement.Value--;
    return TypedResults.Ok(idToDecrement);
});
