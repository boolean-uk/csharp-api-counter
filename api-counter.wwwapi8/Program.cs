...
{
    return TypedResults.Ok(CounterHelper.Counters.Where(x => x.Id == id));
});
...
counters.MapGet("/incremented/{id}", (int id) =>
{
    Counter counter = CounterHelper.Counters.First(x => x.Id == id);
    counter.Value++;
...
counters.MapGet("/decremented/{id}", (int id) =>
{
    Counter counter = CounterHelper.Counters.First(x => x.Id == id);
    counter.Value--;
    return TypedResults.Ok(counter);
