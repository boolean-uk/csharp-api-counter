...
counters.MapGet("/{id}", (int id) =>
{
    ...
    if (counter != null) return Results.Ok(counter);
...
{
    List<Counter> resultingCounters = CounterHelper.Counters.Where(counter => counter.Value > number).ToList();
...
counters.MapPost("/increment/{id}", (int id) =>
{
    ...
    {
        counter.Value++;
        return TypedResults.Created($"https://localhost:7065/counters/{id}", counter);
    }
