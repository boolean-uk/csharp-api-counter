...
{
    return TypedResults.Ok(CounterHelper.CountersGreaterThan(number));
});

////TODO:4. write another controlller method that returns counters that have a value less than the {number} passed in.
counters.MapGet("/lessthan/{number}", (int number) =>
{
    return TypedResults.Ok(CounterHelper.LessThan(number));
});
...
counters.MapPut("/increment/{id}", (int id) =>
    {
        return TypedResults.Ok(CounterHelper.Increment(id));
    });
...
{
    return TypedResults.Ok(CounterHelper.Decrement(id));
});
