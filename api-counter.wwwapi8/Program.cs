...
counters.MapGet("/{id}", (int id) =>
{
    ...
    if (counter != null) return TypedResults.Ok(counter);
...
counters.MapGet("/greaterthan/{number}", (int number) =>
{
    ...
    if (counters.Any()) return TypedResults.Ok(counters);
    return Results.NotFound($"No values greater than {number} found!");
});
...
counters.MapGet("/lessthan/{number}", (int number) =>
{
    var counters = CounterHelper.Counters.Where(c => c.Value < number);
    if (counters.Any()) return TypedResults.Ok(counters);
    return Results.NotFound($"No values less than {number} found!");
});
