...
counters.MapGet("/greaterthan/{number}", (int number) =>
{
    ...
    if (counter == null)
    {
        return Results.NotFound($"Id: {number} not found!");
    }
    return TypedResults.Ok(counter);
});
...
counters.MapPut("/increment/{id}", (int id) => {
    var existingCounter = CounterHelper.Counters.FirstOrDefault(c => c.Id == id);
    ...
    return TypedResults.Ok(existingCounter); //200 or 204 NoContent
});
